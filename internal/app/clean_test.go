...
func TestCleanEndToEndDelete(t *testing.T) {
	...
	sum, err := app.Clean(context.Background(), cfg, discardLogger(), nil, nil)
	if err != nil {
...
func TestCleanPreviewThenCommit(t *testing.T) {
	...
	preview, err := app.Clean(context.Background(),
		config.CleanConfig{Source: src, DestRoot: dst, Delete: false}, discardLogger(), nil, nil)
	if err != nil {
	...
	commit, err := app.Clean(context.Background(),
		config.CleanConfig{Source: src, DestRoot: dst, Delete: true}, discardLogger(), nil, nil)
	if err != nil {
...
func TestCleanRemovesCompanionsAfterSort(t *testing.T) {
	...
	cfg.Sidecars = true
	if _, err := app.Organize(context.Background(), cfg, discardLogger(), nil, nil); err != nil {
